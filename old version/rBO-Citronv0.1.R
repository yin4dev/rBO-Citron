          ...
          box(
            ...
            tabsetPanel(
              ...
              tabPanel(
                ...
                actionButton("optimize", "Execute Optimization", class = "btn-success"),
                hr(),
              ...
              tabPanel(
                ...
                helpText("2. For CSV, upload the file; for Database, ensure data is loaded via the Database tab."),
                helpText("3. Filter data, select objective and explanatory variables, and set constraints."),
                helpText("4. Click 'Execute Optimization' to run Bayesian optimization."),
                helpText("5. In the 'Prediction' tab, choose the prediction mode.")
              )
            )
          )
        )
...
server <- function(input, output, session) {
  ...
  # Reactive for the training data.
  data <- reactive({
    req(input$bayes_data_source)  # Ensure this input is available
  ...
  scaling_info <- reactive({
    req(filtered_data(), input$evs)
    ev_data <- filtered_data()[, input$evs, drop = FALSE]
    info <- list()
    for(col in input$evs) {
      if(is.numeric(ev_data[[col]])) {
        info[[col]] <- list(min = min(ev_data[[col]], na.rm = TRUE),
                            max = max(ev_data[[col]], na.rm = TRUE))
      } else {
        info[[col]] <- list(min = 1, max = length(unique(ev_data[[col]])))
      }
    }
    info
  })
  
  scale_grid <- function(df, scaling) {
    df_scaled <- df
    for(col in colnames(df)) {
      if(col %in% names(scaling)) {
        min_val <- scaling[[col]]$min
        max_val <- scaling[[col]]$max
  ...
  gp_model <- reactive({
    ...
    for(col in input$evs) {
      if (!is.numeric(ev_data[[col]])) {
        levs <- sort(unique(as.character(ev_data[[col]])))
        ev_data[[col]] <- as.numeric(factor(ev_data[[col]], levels = levs))
      }
    }
    X <- as.matrix(ev_data)
    sc <- scaling_info()
    ...
    for(i in 1:ncol(X)) {
      ...
      X_scaled[, i] <- (X[, i] - sc[[colname]]$min) / (sc[[colname]]$max - sc[[colname]]$min)
    }
    if (any(is.na(obj_var)) || any(is.na(X_scaled))) return(NULL)
    if (any(apply(X_scaled, 2, function(x) length(unique(x))) == 1)) return(NULL)
    tryCatch({
  ...
  observeEvent(input$optimize, {
    withProgress(message = "Optimization in progress. Please wait...", value = 0, {
      ...
      obj_var <- filtered_data()[[input$objective]]
      ev_data <- filtered_data()[, input$evs, drop = FALSE]
      if (any(is.na(obj_var)) || any(is.na(ev_data))) {
        showModal(modalDialog(
          title = "Data Error",
          "There are missing values in the filtered data. Please process them first.",
          easyClose = TRUE,
          footer = modalButton("Close")
        ))
        return()
      }
      if (any(apply(ev_data, 2, function(x) length(unique(x))) == 1)) {
        showModal(modalDialog(
          title = "Data Error",
          "One or more explanatory variables are constant. Please adjust filtering or variable selection.",
          easyClose = TRUE,
          footer = modalButton("Close")
        ))
        return()
      ...
      bounds_list <- lapply(input$evs, function(ev) {
        ...
        if (is.numeric(col_data)) {
          ...
          selected_levels <- input[[paste0("levels_", ev)]]
          levs <- sort(unique(as.character(col_data)))
          sapply(selected_levels, function(x) match(x, levs))
        }
      })
      names(bounds_list) <- input$evs
      
      incProgress(0.3, detail = "Generating grid...")
      grid_sizes <- sapply(bounds_list, length)
      total_grid_size <- prod(grid_sizes)
      max_grid_size <- 1e6
      if (total_grid_size > max_grid_size) {
        showModal(modalDialog(
          title = "Grid Size Too Large",
          paste("The number of grid points exceeds", max_grid_size, ". Please adjust the interval or level selections."),
          easyClose = TRUE,
          footer = modalButton("Close")
        ))
        return()
      ...
      grid <- expand.grid(bounds_list)
      
      if (nrow(constraints()) > 0) {
        for (i in 1:nrow(constraints())) {
          expr <- constraints()$Expression[i]
          tryCatch({
            grid <- grid[with(grid, eval(parse(text = expr))), ]
          }, error = function(e) {
            showModal(modalDialog(
      ...
      incProgress(0.4, detail = "Building GP model...")
      if (is.null(gp_model())) {
        showModal(modalDialog(
          title = "Model Fitting Error",
          "GP model cannot be built with the current data and settings.",
          easyClose = TRUE,
          footer = modalButton("Close")
        ))
        return()
      } else {
        optimization_results$gp_model <- gp_model()
      }
      
      gp_mod <- optimization_results$gp_model
      minimize <- (input$target == "Minimization")
      best_y <- if (minimize) min(obj_var) else max(obj_var)
      jitter <- input$jitter
      kappa <- input$kappa
      
      incProgress(0.6, detail = "Performing prediction on entire grid...")
      grid_scaled <- scale_grid(grid, scaling_info())
      predictions_all <- predict.GP(gp_mod, as.matrix(grid_scaled))
      mu_all <- predictions_all$Y_hat
      sigma_all <- sqrt(predictions_all$MSE)
      ci_multiplier <- 1.96
      
      grid$Predicted_OV <- round(mu_all, 3)
      grid$Lower_CI <- round(mu_all - ci_multiplier * sigma_all, 3)
      grid$Upper_CI <- round(mu_all + ci_multiplier * sigma_all, 3)
      
      incProgress(0.7, detail = "Calculating acquisition function...")
      if (input$acqFunc == "EI") {
        acq_val <- expected_improvement(mu_all, sigma_all, best_y, jitter, minimize)
      } else if (input$acqFunc == "PI") {
        acq_val <- probability_improvement(mu_all, sigma_all, best_y, jitter, minimize)
      } else {
        acq_val <- upper_confidence_bound(mu_all, sigma_all, kappa, minimize)
      }
      grid$Acquisition <- acq_val
      
      if (input$acqFunc == "UCB" && minimize) {
        grid_sorted <- grid[order(grid$Acquisition), ]
      } else {
        grid_sorted <- grid[order(-grid$Acquisition), ]
      }
      ...
      if (length(input$plotEVs) == 1) {
        ...
        if (is.numeric(filtered_data()[[ev_name]])) {
          ...
          selected_levels <- input[[paste0("levels_", ev_name)]]
          levs <- sort(unique(as.character(filtered_data()[[ev_name]])))
          codes <- sapply(selected_levels, function(x) match(x, levs))
          grid_var <- data.frame(val = codes)
          names(grid_var) <- ev_name
        }
        for (ev in other_evs) {
          if (is.numeric(filtered_data()[[ev]])) {
            ...
            grid_var[[ev]] <- match(fixed_values[[ev]], levs)
          }
        }
        grid_var <- grid_var[, input$evs, drop = FALSE]
        grid_var_scaled <- scale_grid(grid_var, scaling_info())
        preds_var <- predict.GP(gp_mod, as.matrix(grid_var_scaled))
        grid_var$Prediction <- preds_var$Y_hat
        grid_var$Uncertainty <- sqrt(preds_var$MSE)
        if (input$acqFunc == "EI") {
          grid_var$Acquisition <- expected_improvement(preds_var$Y_hat, sqrt(preds_var$MSE), best_y, jitter, minimize)
        } else if (input$acqFunc == "PI") {
          grid_var$Acquisition <- probability_improvement(preds_var$Y_hat, sqrt(preds_var$MSE), best_y, jitter, minimize)
        } else {
          grid_var$Acquisition <- upper_confidence_bound(preds_var$Y_hat, sqrt(preds_var$MSE), kappa, minimize)
        }
        ...
        makeSeq <- function(ev) {
          if (is.numeric(filtered_data()[[ev]])) {
            ...
            levs <- sort(unique(as.character(filtered_data()[[ev]])))
            codes <- sapply(input[[paste0("levels_", ev)]], function(x) match(x, levs))
            codes
          }
        }
        seq1 <- makeSeq(ev1)
        seq2 <- makeSeq(ev2)
        grid_var <- expand.grid(seq1, seq2)
  ...
  output$predictionUI <- renderUI({
    tagList(
      ...
      conditionalPanel(
        ...
        actionButton("predictButton", "Execute Prediction (Manual)", class = "btn-success"),
      ...
      conditionalPanel(
        ...
        actionButton("predictCSVButton", "Execute Prediction (CSV)", class = "btn-success"),
        downloadButton("downloadPred", "Download Prediction Results as CSV"),
  ...
  observeEvent(input$predictButton, {
    ...
    ev_values <- df[, evs, drop = FALSE]
    for(ev in evs) {
      if (!is.numeric(filtered_data()[[ev]])) {
        levs <- sort(unique(as.character(filtered_data()[[ev]])))
        ev_values[[ev]] <- as.numeric(factor(ev_values[[ev]], levels = levs))
      } else {
        ev_values[[ev]] <- as.numeric(ev_values[[ev]])
      }
    }
    ev_values_scaled <- scale_grid(ev_values, scaling_info())
    predictions <- predict.GP(gp_model(), as.matrix(ev_values_scaled))
  ...
  observeEvent(input$predictCSVButton, {
    req(input$pred_csv, gp_model(), input$evs)
    df_full <- tryCatch({
    ...
    for (ev in required_evs) {
      if (!is.numeric(filtered_data()[[ev]])) {
        levs <- sort(unique(as.character(filtered_data()[[ev]])))
        df_subset[[ev]] <- as.numeric(factor(df_subset[[ev]], levels = levs))
      } else {
        df_subset[[ev]] <- as.numeric(df_subset[[ev]])
      }
    }
    df_scaled <- scale_grid(df_subset, scaling_info())
    predictions <- predict.GP(gp_model(), as.matrix(df_scaled))
    mu <- predictions$Y_hat
    sigma <- sqrt(predictions$MSE)
    ci_multiplier <- 1.96
